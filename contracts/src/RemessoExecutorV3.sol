 ...
 */
contract RemessoExecutorV3 is Ownable2Step, Pausable, ReentrancyGuard {
    ...
    function createSchedule(
        ...
        if (payoutType == PayoutType.Direct) pinnedFee = 0;
        ...
        _schedules[id] = Schedule({
            ...
            amountIn: amountIn,
            minRateE6: minRateE6,
            poolFee: pinnedFee, //  pinned: setPoolFee must not re-route live schedules
            token: token,
            expiresAt: expiresAt,
            payoutType: payoutType,
            active: true,
        ...
        returns (uint256 amountOut)
    {
        ...
        if (s.payoutType == PayoutType.Direct) {
            uint128 direct = s.amountIn;
            address directDest = s.destination;
            address directFrom = s.sender;
            ...
            IERC20(s.token).safeTransferFrom(directFrom, directDest, direct);

            emit RunExecuted(id, directFrom, directDest, direct, direct, s.runsExecuted, s.nextRunAt);
            return direct;
        }
