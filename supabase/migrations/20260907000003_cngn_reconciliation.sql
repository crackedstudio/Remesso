...
--    `reconcile-redemptions` polls the cNGN transactions API for these.
--    Ordered oldest-first so the longest-stuck run is never starved, and
--    bounded because the API allows 20 requests per minute in total.
...
-- How many times reconciliation has looked at this run. Lets the sweep give up
-- and escalate rather than polling a dead reference until the end of time.
alter table public.runs
  add column if not exists reconcile_attempts int not null default 0,
  add column if not exists reconciled_at timestamptz;

-- A sender needs to see *why* a run is still open without reading our logs.
...
-- The hourly reconcile in 20260907000002 pointed at execute-due-runs with a
-- mode flag that function never implemented, so nothing was reconciled. Give it
-- its own function, and run it every 10 minutes: an unretried webhook is the
-- normal way a bank payout goes quiet, not an exotic one.
